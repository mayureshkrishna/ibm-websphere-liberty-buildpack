...
module LibertyBuildpack::Services
  ...
  describe 'Oracle' do
    ...
    describe 'Generate configuration' do
      ...
      #----------------------------------------------------------
      def get_ds_id
        'compose-oracle-myDatabase'
      end

      def get_props_id
        'compose-oracle-myDatabase-props'
      end

      def get_driver_id
        'compose-oracle-driver'
      end

      def get_lib_id
        'compose-oracle-library'
      end

      def get_fileset_id
        'compose-oracle-fileset'
      end
      ...
      def get_oracle_url
        'jdbc:oracle://myHost.com:1521/myDb?useSSL=true&amp;serverSslCert=/home/vcap/app/.compose_oracle/cacert.pem'
      end
      ...
      def check_variables(root, vcap_services)
        ...
        expected_vars << '<variable name=\'cloud.services.myDatabase.connection.port\' value=\'1521\'/>'
        ...
        expected_vars << '<variable name=\'cloud.services.myDatabase.connection.uri\' value=\'oracle://myUser:myPassword@myHost.com:1521/myDb\'/>'
      ...
      def check_server_xml_create(root, sm)
        ...
        t2 = "<properties databaseName='#{get_name}' id='#{get_props_id}' password='#{get_password}' portNumber='#{get_port}' serverName='#{get_host}' url='#{get_oracle_url}' user='#{get_user}'/>"
        t3 = '</dataSource>'
        expected_config << t1 + t2 + t3
        driver_info = "javax.sql.ConnectionPoolDataSource='javax.sql.ConnectionPoolDataSource' javax.sql.XADataSource='javax.sql.XADataSource'"
      ...
      def check_server_xml_update(root, sm)
        ...
        t2 = "<properties databaseName='noName' password='noPassword' portNumber='1521' serverName='noHost' user='noUser'/>"
        t3 = '</dataSource>'
        contents << t1 + t2 + t3
        ...
        t2 = "<properties databaseName='#{get_name}' password='#{get_password}' portNumber='#{get_port}' serverName='#{get_host}' url='#{get_oracle_url}' user='#{get_user}'/>"
        t3 = '</dataSource>'
        expected_config << t1 + t2 + t3
      ...
      it 'on Bluemix (compose for oracle)' do
        vcap_services = {}
        oracle = {}
        oracle['name'] = 'myDatabase'
        oracle['label'] = 'compose-for-oracle'
        oracle_credentials = {}
        oracle_credentials['name'] = 'foobar'
        oracle_credentials['uri'] = 'oracle://myUser:myPassword@myHost.com:1521/myDb'
        oracle_credentials['ca_certificate_base64'] = ''
        oracle['credentials'] = oracle_credentials
        vcap_services['compose-for-oracle'] = [oracle]
