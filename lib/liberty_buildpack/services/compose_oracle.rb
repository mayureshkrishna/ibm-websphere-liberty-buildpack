...
module LibertyBuildpack::Services
  ...
  #------------------------------------------------------------------------------------
  # The ComposeOracle class is the class for Compose for Oracle relational database resources.
  #------------------------------------------------------------------------------------
  class ComposeOracle < Oracle
    ...
    def parse_vcap_services(element, instance)
      ...
      @oracle_url = 'jdbc:' + URI::Generic.build(
        scheme: 'oracle',
        host: uri.host,
        path: uri.path,
        port: uri.port,
    ...
    def modify_properties(properties_element)
      ...
      properties_element.add_attribute('url', @oracle_url)
    end

    private

    CRT_DIRECTORY = '.compose_oracle'.freeze
